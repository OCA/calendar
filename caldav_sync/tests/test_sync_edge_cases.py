...
class TestCalendarAttendeeNotifications(TransactionCase, CaldavTestCommon):
    ...
    def test_notify_attendees_with_dont_notify_context(self):
        """Test that _notify_attendees returns False with dont_notify context."""
        ...
        if attendee:
            ...
            result = attendee.with_context(dont_notify=True)._notify_attendees(
                mail_template
            )
            self.assertFalse(result)

    def test_notify_attendees_without_dont_notify_context(self):
        """Test that _notify_attendees calls super without dont_notify."""
        with patch("caldav.DAVClient"):
            self.user_1._compute_is_caldav_enabled()
            event = (
                self.env["calendar.event"]
        ...
        if attendee:
            ...
            attendee._notify_attendees(mail_template)

...
class Meeting(models.Model):
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            # support v13 constraints
            if "start" not in values:
                values["start"] = values.get("start_datetime", fields.Date.today())
            if "stop" not in values:
                values["stop"] = values.get("stop_datetime", fields.Date.today())
            if "start_datetime" not in values:
                values["start_datetime"] = values.get("start", fields.Date.today())
            if "stop_datetime" not in values:
                values["stop_datetime"] = values.get("stop", fields.Date.today())
    ...
    def unlink(self, can_be_deleted=True):
        ...
        result = super().unlink(can_be_deleted=can_be_deleted)
